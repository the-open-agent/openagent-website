...
Published Store cards also link to an Agent detail page that shows the Store profile, file tree, description, comments, and **Start Chat** action. Users can fork an agent from this page to create their own Store copy. Forking duplicates only the Store configuration; the file tree is not copied and usage counters (chats, messages, vectors) start from zero. Use the detail page as a read-only landing page for users who want to inspect an agent before starting a conversation.

Comments left on Agent detail pages can be moderated from the **Comments** page under **Admin**. It lists every comment with its target, author, content, and created time, and lets admins edit or delete individual comments.
