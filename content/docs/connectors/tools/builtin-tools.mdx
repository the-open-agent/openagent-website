...
For template-based PowerPoint generation, call `pptx_template_analyze` on a local `.pptx` file or chat attachment URL first. It returns slide, text slot, table, chart, and capacity metadata. Then call `pptx_template_fill` with a plan that selects, repeats, reorders, and fills template slides. Template filling validates missing targets, chart data, and new object collisions before writing the output file.
