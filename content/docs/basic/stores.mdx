...
Published Store cards also link to a Store view page that shows the Store profile, file tree, description, and a **Start Chat** action. Use it as a read-only landing page for users who want to inspect an agent before starting a conversation.
...
Stores can show an AI virtual figure in the chat UI. The Store fields control whether the figure is enabled, which image URL it uses, and whether it starts expanded or collapsed. Users can resize, move, collapse, or disable the figure from its chat menu; the figure status follows the chat state, such as typing, thinking, replying, error, or done.
...
OpenAgent exposes OpenAI-compatible chat completion endpoints at `POST /api/chat/completions` and `POST /api/v1/chat/completions`. The Store edit page shows copyable **Base URL** and **Chat completions endpoint** values for external clients.
