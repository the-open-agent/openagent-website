...
<Callout type="warn">
  **Diagram placeholder (Request lifecycle):** Add a single flow diagram for the 5-step lifecycle:
  Load history → Retrieve (optional) → Assemble context → Model/tool loop → Stream & save.
  Include the “tool call loop” visually (model may call tools multiple times).
</Callout>
