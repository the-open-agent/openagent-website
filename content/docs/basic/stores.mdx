...
- The **super admin** (the global admin whose username is `admin`) can set any state directly, including **Published**.
- Other **admins** can set any state except **Published** — they move a Store to **Pending Review** instead of publishing it outright.
...
The Hub lists only Stores whose publish state is **Published**, using profile fields such as **Author**, **Subject**, **Grade**, **Topic**, and **Brief** on the card. Each card also shows the agent's **star**, **watcher**, and **fork** counts.

On the Hub page, users can search published Stores by name, author, owner, or affiliation; filter by **Subject**, **Grade**, and **Topic**; and sort by star, watcher, or fork count, or by display name, author, affiliation, subject, grade, or topic. The default sort is **most starred**. Selecting a card opens a details drawer with the Store profile, description, full chat link, copy-link action, and **Start Chat** button. Signed-in users can also switch the listing between **All agents**, **Starred**, and **Watching** with the view selector, so favorited and followed agents are easy to find again.
...
Published Store cards also link to a public **[Agent page](/docs/agent-page)** — a read-only landing page for inspecting an agent before starting a conversation. It offers Star, Watch, and Fork actions and is organized into tabs for the agent's [Overview](/docs/agent-page/overview), [Chat](/docs/agent-page/chat), [Files](/docs/agent-page/files), [Issues](/docs/agent-page/issues), [Security](/docs/agent-page/security), [Insights](/docs/agent-page/insights), and owner-only [Settings](/docs/agent-page/settings). See the [Agent Page](/docs/agent-page) reference for the full breakdown.
...
A published agent's usage analytics and its automated security audit live on the [Agent page](/docs/agent-page), under the [Insights](/docs/agent-page/insights) and [Security](/docs/agent-page/security) tabs.
