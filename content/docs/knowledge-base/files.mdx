...
The Files list also supports direct uploads from the table toolbar. Uploaded files are stored through the Store's Storage Provider, saved as File records, and queued for vector generation automatically. Image files show a small preview in the list; for other files, use the file URL or the related Vectors view to inspect processed content.
