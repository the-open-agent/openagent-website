...
| Images | `.png`, `.jpg`, `.jpeg`, `.webp` | Can be indexed as knowledge when an image-capable extraction path is configured |
...
## URL and media ingestion

OpenAgent can cache document URL content so repeated ingestion of the same remote resource does not need to fetch and parse it from scratch every time. This is useful for knowledge sources that are referenced by URL from tasks, import flows, or automation.

For image knowledge, use an image-capable model or extraction provider so visual content can be converted into text before embedding. Treat the generated text the same way you would any other extracted document: verify the chunks after processing, then tune the Store's Knowledge Count and prompt if retrieval quality needs adjustment.

## PowerPoint templates

PowerPoint files can be used for more than text extraction. OpenAgent can accept PPTX templates, replace slide images, fill text placeholders, and edit SmartArt text in supported templates. Use this for agents that generate or update presentations as part of an office automation workflow.
