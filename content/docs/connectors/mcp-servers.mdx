...
After a token is saved, API responses return it as `***`. Submit `***` unchanged when editing other fields to keep the existing token; enter a new value only when rotating the credential.
