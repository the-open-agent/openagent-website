...
Available office sub-tools include Word read/write, Excel read/write, `pptx_read`, and `pptx_write`. The PowerPoint writer creates editable `.pptx` files by running a local PptxGenJS `.mjs` build script. The script exports `build(pptx, ctx)`, adds slides through PptxGenJS, and can receive optional JSON `data` plus an `assets_dir` for images or icons.
