...
API chat sessions created through a Store's OpenAI-compatible external API key are marked as API chats. Their Chat and Message records are visible for audit and cost tracking, but are read-only in the admin UI so external-client history cannot be edited after the fact.
