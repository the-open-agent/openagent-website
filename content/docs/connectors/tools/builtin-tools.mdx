...
<Callout type="info">
  Older deployments may refer to `local_documents_scan`, `local_document_read`, or `local_text_write`. Current OpenAgent exposes the generic `local_file_scan`, `local_file_read`, and `local_file_write` APIs instead.
</Callout>
