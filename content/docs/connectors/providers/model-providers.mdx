...
| **Ollama** | Local LLM execution and chat completions (Llama, Mistral, Gemma, etc.). |
...
| **OpenCode** | Coding-agent provider integration for OpenCode-compatible workflows. |
...
<Tabs items={['Standard APIs', 'Azure OpenAI', 'Amazon Bedrock', 'Ollama / Local', 'OpenAI Compatible']}>
  <Tab value="Standard APIs">
    Most providers (OpenAI, Claude, DeepSeek, OpenRouter, MiniMax) follow this pattern:
    
    - **Type**: Select the provider name.
    - **Sub Type**: The specific model ID (e.g., `gpt-4o`, `deepseek-chat`).
    - **API Key**: Your secret token.

    Some providers use provider-specific credential labels. For MiniMax, use the MiniMax group ID and API key fields exposed in the Provider form.
  </Tab>
...
The Provider form includes quick setup entries for common model providers and popular models. Use these presets to fill the Type and Sub Type fields, then add your credentials and test the provider before attaching it to a Store.

Quick setup is especially useful for providers with fast-moving model lists, including MiniMax, local/Ollama deployments, OpenAI-compatible services, and multimodal providers.
...
- **Multimodal Models**: Vision-capable model families such as Qwen-VL can process image inputs when the selected provider and Store workflow support them.
