...
description: Where uploaded file bytes are physically stored — local disk or OpenAI Files API.
---
...
Use **Local File System** unless you have a specific reason to offload file storage to another backend.
...
<Callout type="warn">
  **Screenshot placeholder:** Providers → Add Provider (Storage) selection (Local vs OpenAI).
  Keep it brief; storage should feel like a “set once and forget” setting.
</Callout>
...
Files are uploaded to the OpenAI Files API and stored there.

- **Best for**: deployments already using OpenAI infrastructure that want file storage managed externally
- **API Key required**: your OpenAI API key
- **Cost**: OpenAI Files API pricing applies

The original file bytes are sent to OpenAI during upload. Processing (text extraction, chunking, embedding) still happens within OpenAgent — the storage provider only determines where the file bytes land.

## Adding a provider

1. **Providers → Add Provider**, category **Storage**
2. Choose **Local File System** or **OpenAI File System**
3. For OpenAI File System, enter your OpenAI **API Key**
4. Save
...
In practice, most deployments use **Local File System** throughout. Switch to **OpenAI File System** only if you have a specific reason to offload storage to OpenAI's infrastructure.
