...
| **API Key** | Store-scoped key for the OpenAI-compatible chat completions API. Keys are generated automatically for new Stores and masked after saving. |
...
OpenAgent exposes an OpenAI-compatible `POST /api/chat/completions` endpoint. Use `Authorization: Bearer <key>` with either:

- a Store **API Key**, which routes the request through that Store's model provider and records the API conversation as Chat and Message records with user `api`
