...
<Steps>
  <Step>
    ...
    **Windows** — download `openagent-windows-x86.exe` and double-click it, or run from PowerShell:
    ```powershell
    .\openagent-windows-x86.exe
    ```
    No WSL, no Docker, no additional dependencies. Runs natively on Windows 10/11.

    **macOS / Linux** — download the binary for your platform and run:
    ```bash
    chmod +x openagent-linux-x86
    ./openagent-linux-x86
    ```
