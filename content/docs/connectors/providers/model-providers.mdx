...
| **OpenCode** | Delegate coding tasks to an OpenCode-compatible agent endpoint. |
...
The Provider page includes a **Quick Setup** flow for common model providers. It pre-fills the provider type, default model, and provider-specific fields so you can add a working model with fewer manual choices.

Quick Setup supports providers including OpenAI, Claude, Gemini, DeepSeek, Grok, Ollama, OpenRouter, Mistral, MiniMax, Azure, OpenAI Compatible, Alibaba Cloud, Moonshot, Silicon Flow, Volcano Engine, Baidu Cloud, Amazon Bedrock, Hugging Face, iFlytek, ChatGLM, Cohere, Baichuan, StepFun, Tencent Cloud, Yi, GitHub Models, Writer, Local, and OpenCode.
