...
The Files list also supports direct uploads from the table toolbar. Uploaded files are stored through the default Storage Provider, attached to the user's default Store, and saved as File records. Filenames are stored under a generated object path so two uploads with the same filename do not overwrite each other.
...
Scanned PDFs may need OCR before their text can be read. For agent-side reading of scanned local PDFs, enable the `local_file` Tool and use `local_pdf_ocr_read`; see [Built-in Tools](/docs/connectors/tools/builtin-tools#local_file) for the OCR endpoint configuration.
