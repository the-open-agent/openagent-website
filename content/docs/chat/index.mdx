...
**5. Stream and save.** Tokens stream to the client in real time. The UI also shows generation status while the model retrieves context, calls tools, or waits on provider output. When the response is complete, a Message record is saved containing the text, tool calls, retrieval metadata (if any), token counts, and estimated cost.
...
The chat input accepts text, uploaded files, pasted files, and pasted images when the Store allows uploads. This is useful for quick analysis workflows where a user wants to paste a screenshot, document, or local file into the current conversation without first adding it to the long-term knowledge base.

Use **Disable File Upload** on the Store when a chat should be text-only.

## Voice and virtual figure

When a Store has Speech-to-Text configured, users can dictate messages from the chat input. OpenAgent appends the transcript to the current draft and leaves the user in control of when to send it. When Text-to-Speech is configured, replies can be played back automatically or manually depending on the Store's chat UI settings.

Deployments can also configure an AI virtual figure for the chat experience. The default static figure asset is embedded in release binaries so the chat UI works in offline single-binary deployments.
...
OpenAgent can expose a Store through a v1 OpenAI-compatible chat completions endpoint. External clients can send chat completion requests, and OpenAgent still records the resulting Chats and Messages for auditing, usage accounting, and troubleshooting.

Use this when you want an existing OpenAI-compatible SDK or tool to talk to an OpenAgent Store while keeping the Store's model provider, system prompt, knowledge retrieval, and tool configuration centralized.
...
- Search results and whether web search was enabled for the turn
