...
<Callout type="warn">
  **Screenshot Placeholder:** The Store edit form highlighting the "Split Provider" dropdown with the QA option selected.
</Callout>
...
**Builtin Tools**  
A list of built-in tool types to enable for this Store. Options include `web_search`, `web_browser`, `shell`, `local_file`, `office`, `web_fetch`, `gui`, `video_download`, `browser_use`, and `time`. See [Built-in Tools](/docs/connectors/tools/builtin-tools) for the full reference.
...
<Callout type="warn">
  **Screenshot Placeholder:** The "Forbidden Words" configuration area in the Store settings.
</Callout>
...
<Callout type="warn">
  **Screenshot Placeholder:** The Store configuration UI showing the "Child Stores" selection interface.
</Callout>
